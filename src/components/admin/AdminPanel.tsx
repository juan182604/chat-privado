...
export function AdminPanel() {
  ...
  const [resetPinTarget, setResetPinTarget] = useState<AdminUser | null>(null)
                    ...
                    >
                      {u.role !== 'super_admin' && (
                        <>
                          ...
                          <button
                            onClick={() => { setResetPinTarget(u); setNewPinInput('') }}
                            className="px-2 py-1 text-xs rounded bg-zinc-800 hover:bg-zinc-700 text-blue-400 flex items-center gap-1"
                          >
                            <KeyRound className="w-3 h-3" /> Resetear PIN
                          </button>
                          <button
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-zinc-100">Resetear PIN de @{resetPinTarget.username}</h3>
            </div>
            <p className="text-xs text-zinc-400 mb-3">
              Introduce el nuevo PIN de 6 dígitos para este usuario. Se cerrarán todas sus sesiones activas.
            </p>
            <input
              type="text"
              inputMode="numeric"
              ...
              placeholder="000000"
              className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-3 text-2xl tracking-[0.5em] text-center outline-none focus:border-blue-500 text-zinc-100"
            />
            <div className="flex gap-2 mt-4">
              <button
                onClick={() => { setResetPinTarget(null); setNewPinInput('') }}
                className="flex-1 py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-sm text-zinc-300"
              >
                Cancelar
              </button>
              <button
                onClick={async () => {
                  if (newPinInput.length !== 6) { alert('El PIN debe ser 6 dígitos'); return }
                  const res = await fetch('/api/admin/user', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ targetId: resetPinTarget.id, action: 'reset_pin', newPin: newPinInput }),
                  })
                  const data = await res.json()
                  if (res.ok) {
                    alert(`PIN de ${resetPinTarget.username} reseteado a: ${newPinInput}`)
                ...
                className="flex-1 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-sm text-white font-semibold"
              >
                Resetear PIN
              </button>
            </div>
