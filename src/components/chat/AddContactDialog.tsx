    ...
    <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-3">
          <h2 className="font-semibold text-zinc-100 flex items-center gap-2"><UserPlus className="w-4 h-4" /> Agregar contacto</h2>
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          {found && (
            <div className="bg-zinc-800/60 border border-zinc-700 rounded-lg p-4 flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-emerald-500 to-cyan-500 flex items-center justify-center text-white font-bold text-lg">{found.firstName.charAt(0).toUpperCase()}</div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-zinc-100">{found.username}</p>
                <p className="text-xs text-zinc-400 truncate">{found.firstName} {found.lastName}</p>
                <p className="text-[11px] text-emerald-400 font-mono">#{found.uniqueId}</p>
              </div>
              <button onClick={add} disabled={adding} className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 px-3 py-1.5 rounded-lg text-xs text-white flex items-center gap-1">{adding ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} Agregar</button>
            </div>
          )}
